      ...
      <div className={styles.works}>
        ...
        {deleteNotificationMsg && (
          <DeleteNotificationMsg
            ...
            setDeleteNotificationMsg={setDeleteNotificationMsg}
          />
        )}
        {props.worksArray && (
          <div className={styles.worksGrid}>
            {props.worksArray.map((element, index) => (
              <Link
                href={`${pathname}/${element._id}`}
                key={index}
                style={{ animationDelay: `${index * 100}ms` }}
                className={styles.workElement}
              >
                <img
                  src={process.env.NEXT_PUBLIC_IMAGES_SRC + element.thumbnail}
                  alt={`Sculpture ${element.name}`}
                />
              </Link>
            ))}
          </div>
